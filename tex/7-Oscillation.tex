...
The average measured \Ac rate per baseline, relative to the first baseline, can be seen in Figure~\ref{fig:fidratevsbaselineoscdata}.
Unlike the data set used in the previous chapter, the data for this oscillation analysis is statistics limited, accounting for the much larger error bars than were present in Figure~\ref{fig:fidratevsbaseline}.
...
This distribution was then fit with a $1/r^2$ function, producing a $\chi^2/NDF =  8.351/13$.

\begin{figure}[h]
	\centering
	\includegraphics[width=0.8\linewidth]{tex/7-oscillation-images/FidRateVsBaseline_OscData}
	\caption{The \Ac rate (for fiducial $z$) averaged over each of the 14 defined baselines, plotted relative to the first baseline. Error bars are statistical, systematic errors are shown as the shaded region.}
