...
\begin{figure}[!b]
	\centering
	\includegraphics[width=0.6\linewidth]{tex/7-oscillation-images/AD_Segments_Baselines}
...
The average measured \Ac rate per baseline, relative to the mean over all baselines, can be seen in Figure~\ref{fig:fidratevsbaselineoscdata}.
...
This distribution was then fit with a $1/r^2$ function, producing a $\chi^2/NDF =  8.722/13$, slightly better than before the correction.

\begin{figure}[!t]
	\centering
	\includegraphics[width=0.8\linewidth]{tex/7-oscillation-images/FidRateVsBaseline_OscData}
	\caption{The \Ac rate (for fiducial $z$) averaged over each of the 14 defined baselines, plotted relative to the mean. Error bars are statistical, systematic errors are shown as the shaded region.}
	\label{fig:fidratevsbaselineoscdata}
\end{figure}

\begin{figure}[h]
	\centering
	\includegraphics[width=0.8\linewidth]{tex/7-oscillation-images/IBDCountsVsBaseline_Corrected}
	\caption{Background subtracted IBD event rate as a function of distance from the reactor center binned into 14 unique baselines, corrected for the measured \Ac rates in Figure~\ref{fig:fidratevsbaselineoscdata}.}
